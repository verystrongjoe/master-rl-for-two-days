...
from reference.poisson import Poisson
...
class PolicyIterationSolver(object):
    ...
    def transition_probabilty(self, s, req, ret, action=0):
        ...
        p_req = Poisson.pmf_series(req, s)  # 이건 고객이 리퀘스트를 하는 경우인데?
        p_ret = Poisson.pmf_series(ret, _ret_sz)  # 그날 리턴까지 고려하고 잭이 최대 5대 넣었다고 치면, 렌탈 가능 대수가 20대이니까 26이라고 생각해야하나?
        p = np.outer(p_req, p_ret)
        transp = np.asarray([p.trace(offset) for offset in range(-s, _ret_sz + 1)])  # 여긴 state transition probability에 대해서 찾아와야 한다. p.trace라는건 diagonal을 구하는건데 여기에서 이게 무슨 의미인건지.
