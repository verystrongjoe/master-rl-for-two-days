...
class PolicyIterationSolver(object):
    ...
    def transition_probabilty(self, s, req, ret, action=0):
        ...
        p_req = Poisson.pmf_series(req, s) # request
        p_ret = Poisson.pmf_series(ret, _ret_sz) # return
        ...
        if action > 0:
            ...
            # G1의 스테이트가 아니라 G2의 스테이트라고 하더라도 초기에 action 개수만큼 이상은 있을거니까 g2의 스테이트임을 얘기하는거 같은데
        ...
        return np.roll(transp, shift=-action)[:self.capacity + 1] # 이건 오히려 뒤를 비우는거니까.. 2에서 1로 갔응꼐
